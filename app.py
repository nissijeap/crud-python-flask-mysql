...
def Index():
    ...
    alldata = cur.fetchall()
     ...
     # Fetch the latest highest price of Bitcoin
    cur.execute("""
        SELECT MAX(cp.highest_price)
        FROM cryptocurrency_prices cp
        JOIN cryptocurrency_types ct ON cp.cryptotype = ct.id
        WHERE ct.name = 'Bitcoin'
    """)
    bitcoin_highest = cur.fetchone()[0]
    
    # Fetch the latest highest price of Ethereum
    cur.execute("""
        SELECT MAX(cp.highest_price)
        FROM cryptocurrency_prices cp
        JOIN cryptocurrency_types ct ON cp.cryptotype = ct.id
        WHERE ct.name = 'Ethereum'
    """)
    ethereum_highest = cur.fetchone()[0]
    
     # Fetch the latest highest price of Tether
    cur.execute("""
        SELECT MAX(cp.highest_price)
        FROM cryptocurrency_prices cp
        JOIN cryptocurrency_types ct ON cp.cryptotype = ct.id
        WHERE ct.name = 'Tether'
    """)
    tether_highest = cur.fetchone()[0]
    
    # Fetch the latest highest price of Shiba Inu
    cur.execute("""
        SELECT MAX(cp.highest_price)
        FROM cryptocurrency_prices cp
        JOIN cryptocurrency_types ct ON cp.cryptotype = ct.id
        WHERE ct.name = 'Solana'
    """)
    solana_highest = cur.fetchone()[0]
    ...
    cur.execute("""
        ...
        FROM cryptocurrency_prices cp
        JOIN cryptocurrency_types ct ON cp.cryptotype = ct.id
        WHERE ct.name = 'Litecoin'
    """)
    litecoin_highest = cur.fetchone()[0]
    ...
    cur.execute("""
        ...
        FROM cryptocurrency_prices cp
        JOIN cryptocurrency_types ct ON cp.cryptotype = ct.id
        WHERE ct.name = 'Dogecoin'
    """)
    dogecoin_highest = cur.fetchone()[0]
    
    
    cur.close()

    return render_template('dashboard.html', cryptocurrency_prices=data, 
                       cryptocurrency_types=crypto_types, alldata=alldata,  
                       bitcoin_highest=bitcoin_highest, ethereum_highest=ethereum_highest, 
                       tether_highest=tether_highest, solana_highest=solana_highest, 
                       litecoin_highest=litecoin_highest, dogecoin_highest=dogecoin_highest)
